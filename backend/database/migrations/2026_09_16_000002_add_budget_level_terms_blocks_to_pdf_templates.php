...
/**
 * Condições comerciais por nível de manutenção: leva aos modelos de orçamento
 * já publicados os dois blocos novos — "Diferenciais desta opção" (logo
 * depois de "Opção de manutenção") e "Entrega" (logo depois de "Garantia").
 *
 * Cirúrgico como 2026_09_15_000003: cada bloco é condicional (invisível
 * quando a variável vem vazia, ou seja, em orçamento comum), cada um é
 * inserido só se o marcador que o antecede existir e ainda não houver o
 * bloco. O resto do modelo, inclusive personalizações, fica intocado.
 */
return new class extends Migration
{
    public function up(): void
    {
        ...
        foreach ($templates as $template) {
            ...
            DB::transaction(function () use ($template): void {
                ...
                if (! str_contains($corpoJson, 'orcamento.beneficios_texto')) {
                    $novo = $this->insertAfterMarker($corpo, 'orcamento.opcao_texto', PdfDefaultTemplates::blocoBeneficiosOpcao());
                    if ($novo !== null) {
                        $corpo = $novo;
                        $mudou = true;
                    }
                ...
                if (! str_contains($corpoJson, 'orcamento.entrega_domicilio_texto')) {
                    $novo = $this->insertAfterMarker($corpo, 'orcamento.garantia_texto', PdfDefaultTemplates::blocoEntregaDomicilio());
                    if ($novo !== null) {
                        $corpo = $novo;
                        $mudou = true;
                    }
    ...
    /**
     * Insere `$blocos` logo depois do bloco de topo do corpo que contém o
     * marcador (uma variável). Devolve null quando o marcador não existe —
     * modelo personalizado que removeu a seção fica como está.
     *
     * @param  array<int, mixed>  $corpo
     * @param  array<int, array<string, mixed>>  $blocos
     * @return array<int, mixed>|null
     */
    private function insertAfterMarker(array $corpo, string $marcador, array $blocos): ?array
    {
        $posicao = null;
