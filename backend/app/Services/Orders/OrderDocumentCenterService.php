...
class OrderDocumentCenterService
{
    ...
    public function catalog(int $orderId, User $actor): array
    {
        ...
        return [
            ...
            'whatsapp_templates' => $this->availableWhatsappTemplates($order),
    ...
    public function queueSend(int $orderId, User $actor, array $documentIds, array $payload = []): array
    {
        ...
        $templateCode = $this->resolveRequestedTemplateCode($documents, $payload);
    ...
    public function syncAfterClosure(int $orderId, User $actor): void
    {
        try {
            ...
            // Os três "reparo entregue" (pago, sem custo, garantia) geram
            // comprovante de entrega. A cobrança só sai quando há valor
            // (valor_final > 0) — sem custo/garantia (R$0) não geram cobrança.
            if (in_array($status, OrderStatus::REPAIRED_DELIVERY_CODES, true)) {
            ...
            if ($this->hasTechnicalContent($order) && in_array($status, ['reparo_concluido', ...OrderStatus::REPAIRED_DELIVERY_CODES, 'devolvido_sem_reparo', 'irreparavel', 'cancelado'], true)) {
    ...
    public function syncAfterStatusChange(int $orderId, User $actor, ?string $newStatus = null): void
    {
        try {
            ...
            if ($this->hasTechnicalContent($order) && in_array($status, ['reparo_concluido', ...OrderStatus::REPAIRED_DELIVERY_CODES, 'devolvido_sem_reparo', 'irreparavel'], true)) {
    ...
    private function buildCatalog(Order $order, Collection $documents): array
    {
        ...
        foreach ($documentTypes as $type => $definition) {
            ...
            $catalog[] = [
                ...
                'template_code' => $this->resolveTemplateCodeForDocumentType($type),
    ...
    private function generationPrecondition(Order $order, string $type): array
    {
        ...
        return match ($type) {
            ...
            'entrega' => in_array(mb_strtolower(trim((string) ($order->status ?? ''))), OrderStatus::REPAIRED_DELIVERY_CODES, true)
                ? ['ok' => true, 'reason' => '']
                : ['ok' => false, 'reason' => 'O comprovante de entrega só fica disponível quando a OS é encerrada como equipamento reparado entregue (pago, sem custo ou garantia).'],
    ...
    {
        $requested = trim((string) ($payload['template_code'] ?? ''));
        if ($requested !== '' && $this->activeWhatsappTemplateExists($requested)) {
            return $requested;
        }

        return $this->resolveSuggestedTemplateCode($documents);
    }
    ...
    private function buildDefaultOutboundMessage(Order $order, Collection $documents, string $templateCode): string
    {
        ...
        if ($templateMessage !== '') {
            return $this->renderWhatsappTemplateMessage($order, $templateMessage);
        }
     ...
     */
    private function availableWhatsappTemplates(Order $order): array
    {
        ...
        return WhatsappTemplate::query()
            ...
            ->orderBy('nome')
            ->orderBy('codigo')
            ->get(['codigo', 'nome', 'evento', 'conteudo'])
            ->map(function (WhatsappTemplate $template) use ($order): array {
                $content = trim((string) ($template->conteudo ?? ''));

                return [
                    'code' => trim((string) ($template->codigo ?? '')),
                    'name' => trim((string) ($template->nome ?? '')),
                    'event' => trim((string) ($template->evento ?? '')),
                    'rendered_message' => $content !== '' ? $this->renderWhatsappTemplateMessage($order, $content) : '',
                ];
            })
            ->filter(fn (array $template): bool => ($template['code'] ?? '') !== '' && ($template['name'] ?? '') !== '')
            ->values()
            ->all();
    }

    private function activeWhatsappTemplateExists(string $templateCode): bool
    {
        if ($templateCode === '' || ! Schema::hasTable('whatsapp_templates')) {
            return false;
        }
    ...
    private function renderWhatsappTemplateMessage(Order $order, string $templateMessage): string
    {
        $rendered = strtr($templateMessage, $this->outboundPlaceholderMap($order));

        return preg_replace('/\R{3,}/', "\n\n", trim($rendered)) ?? trim($rendered);
    }

    /**
     * @return array<string, string>
     */
    private function outboundPlaceholderMap(Order $order): array
    {
        $equipment = $order->equipment;
        $equipmentType = trim((string) ($equipment?->type?->nome ?? $equipment?->tipo ?? ''));
        $equipmentBrand = trim((string) ($equipment?->brand?->nome ?? $equipment?->marca ?? ''));
        $equipmentModel = trim((string) ($equipment?->model?->nome ?? $equipment?->modelo ?? ''));
        $equipmentSerial = trim((string) ($equipment?->numero_serie ?? $equipment?->serial ?? $equipment?->imei ?? ''));
        $statusAtual = trim((string) ($order->status ?? $order->status_atual ?? ''));
        $dataAbertura = $this->formatTemplateDate($order->created_at ?? null);
        $dataEntrega = $this->formatTemplateDate($order->updated_at ?? null);
        $valorFinal = $order->valor_final ?? $order->valor ?? null;
        $tecnicoNome = trim((string) ($order->technician?->nome ?? $order->technician?->name ?? ''));
        $clienteNome = trim((string) ($order->client?->nome_razao ?? ''));
        $clienteTelefone = trim((string) ($order->client?->telefone1 ?? $order->client?->telefone_contato ?? ''));
        $clienteEmail = trim((string) ($order->client?->email ?? ''));
        $relatoCliente = trim((string) ($order->relato ?? $order->relato_cliente ?? ''));
        $prioridade = trim((string) ($order->prioridade ?? ''));

        return [
            '{{numero_os}}' => trim((string) ($order->numero_os ?? '')),
            '{{cliente_nome}}' => $clienteNome,
            '{{cliente_telefone}}' => $clienteTelefone,
            '{{cliente_email}}' => $clienteEmail,
            '{{equipamento}}' => $this->resolveEquipmentLabel($order),
            '{{equipamento_tipo}}' => $equipmentType,
            '{{equipamento_marca}}' => $equipmentBrand,
            '{{equipamento_modelo}}' => $equipmentModel,
            '{{equipamento_serie}}' => $equipmentSerial,
            '{{status_atual}}' => $statusAtual,
            '{{data_abertura}}' => $dataAbertura,
            '{{data_entrega}}' => $dataEntrega,
            '{{valor_final}}' => is_numeric($valorFinal) ? number_format((float) $valorFinal, 2, ',', '.') : '',
    ...
    private function formatTemplateDate(mixed $value): string
    {
