...
class ProfilePhotoImageService
{
    ...
    public function update(User $user, UploadedFile $file, User $actor): string
    {
        ...
        if ($previousPath !== '' && $previousPath !== $path) {
            if (! $this->retireCatalogedFile($previousPath, (int) $actor->id)) {
    ...
    public function remove(User $user, User $actor): void
    {
        ...
        if ($path !== '') {
            if (! $this->retireCatalogedFile($path, (int) $actor->id)) {
                Storage::disk('local')->delete($path);
            }
        }
    }
    ...
    /**
     * Move a foto catalogada para a lixeira preservando o binário até o
     * expurgo configurado. Retorna false apenas quando o caminho não pertence
     * ao catálogo ou já está em um estado terminal; falhas de transição mantêm
     * o binário para evitar um registro catalogado sem conteúdo.
     */
    private function retireCatalogedFile(string $storagePath, int $actorId): bool
    {
        ...
        if ($managedFile->lifecycle_status->value === 'trashed') {
            return true;
        }
        if (! in_array($managedFile->lifecycle_status->value, ['active', 'archived'], true)) {
            return false;
        }
        ...
        try {
            ...
            report($exception);
            // O arquivo já pertence ao catálogo: uma falha de transição não
            // autoriza apagar o binário e criar um registro ativo quebrado.
            return true;
        }
    }
