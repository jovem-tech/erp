...
@section('content')
    ...
    <div class="surface-card file-selection-bar p-2 px-3 mb-3 d-flex flex-wrap align-items-center justify-content-between gap-2">
        <div class="d-flex align-items-center gap-2">
            ...
            @if ($selectedLifecycle !== 'trashed' && ! $canDelete)
                        ...
                        <article class="surface-card file-card h-100">
                            <div class="file-preview">
                                <label class="file-select-wrap" title="Selecionar arquivo">
                                    <input class="form-check-input file-select m-0" type="checkbox" value="{{ $file['uuid'] }}" data-downloadable="{{ $deliverable ? '1' : '0' }}" data-trashable="{{ ! $inTrash ? '1' : '0' }}" data-restoreable="{{ $inTrash ? '1' : '0' }}" data-purgeable="{{ $inTrash ? '1' : '0' }}" aria-label="Selecionar {{ $file['safe_download_name'] ?? 'arquivo' }}">
                                </label>
                            ...
                            <div class="p-3">
                                ...
                                <div class="d-flex gap-1">
                                    ...
                                    @endif
                                    @if ($inTrash)
                                        <button type="button" class="btn btn-outline-success btn-sm file-restore-one" data-file-uuid="{{ $file['uuid'] }}" data-file-name="{{ $file['safe_download_name'] ?? 'arquivo' }}" @disabled(!$canRestore || !$mutationsEnabled) title="Restaurar"><i class="bi bi-arrow-counterclockwise"></i></button>
