...
class FileManagerTest extends TestCase
{
    ...
    public function test_missing_trash_content_is_identified_and_cannot_be_selected_for_restore(): void
    {
        $uuid = '019f7c54-fd90-7cc1-a455-aa6f3efd15d1';
        Http::fake([
            '*/api/v1/file-manager/dashboard*' => Http::response($this->success([
        ...
        $response = $this->withSession($this->desktopSession())
            ->get('/arquivos?lifecycle_status=trashed&view=grid');

        $response->assertOk()
            ->assertSee('Conteúdo ausente')
            ->assertSee('data-restoreable="0"', false)
            ->assertSee('title="Não é possível restaurar: conteúdo ausente"', false);
    }
