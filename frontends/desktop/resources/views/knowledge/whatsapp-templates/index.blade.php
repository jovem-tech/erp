...
@section('content')
    <section class="desktop-form-card mb-4">
        <div class="surface-card-header">
            ...
            <div class="d-flex flex-wrap gap-2">
                @if (\App\Support\DesktopSession::can('conhecimento', 'criar'))
                    <a href="{{ route('knowledge.whatsapp-templates.create') }}" class="btn btn-primary">
        ...
        <form method="get" class="desktop-filter-grid">
            ...
            <div class="field-actions" style="grid-column: span 2;">
                ...
                <a href="{{ route('knowledge.whatsapp-templates.index') }}" class="btn btn-outline-light">Limpar</a>
            </div>
        </form>
    </section>
    ...
    <section class="surface-table">
        <div class="surface-table-header">
            <div>
                <h2 class="surface-title">Templates de WhatsApp cadastrados</h2>
                <p class="surface-subtitle">
                    {{ number_format((int) ($pagination['total'] ?? 0), 0, ',', '.') }} templates retornados pela API central.
                </p>
            </div>
                ...
                <table class="table table-stack align-middle">
                    ...
                    @foreach ($templates as $template)
                        ...
                        <tr>
                            ...
                            <td data-label="Evento">{{ $template['evento'] !== '' ? $template['evento'] : '—' }}</td>
                                ...
                                <div class="dropdown supplier-actions-dropdown">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-end supplier-actions-menu">
                                        @if (\App\Support\DesktopSession::can('conhecimento', 'editar'))
                                            <li>
                                                <a href="{{ route('knowledge.whatsapp-templates.edit', $templateId) }}" class="dropdown-item">
                                        ...
                                        @if (\App\Support\DesktopSession::can('conhecimento', 'editar'))
                                            <li>
                                                <form
                                                    method="post"
                                                    action="{{ route('knowledge.whatsapp-templates.toggle-active', $templateId) }}"
                                                    data-confirm="Deseja {{ $active ? 'desativar' : 'ativar' }} este template de WhatsApp?"
                                                    data-confirm-title="{{ $active ? 'Desativar' : 'Ativar' }} template de WhatsApp"
                                        ...
                                        @if (\App\Support\DesktopSession::can('conhecimento', 'excluir'))
                                            <li>
                                                <form
                                                    method="post"
                                                    action="{{ route('knowledge.whatsapp-templates.destroy', $templateId) }}"
                                                    data-confirm="Deseja excluir este template de WhatsApp? Esta ação não pode ser desfeita."
                                                    data-confirm-title="Excluir template de WhatsApp"
        ...
        @else
            @include('layouts.partials.empty-state', [
                'icon' => 'bi-whatsapp',
                'title' => 'Nenhum template de WhatsApp encontrado',
                'message' => 'Ajuste os filtros ou cadastre o primeiro template para automatizar mensagens de WhatsApp.',
            ])
        @endif
    </section>
@endsection
