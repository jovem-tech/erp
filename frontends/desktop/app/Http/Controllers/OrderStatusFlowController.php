...
class OrderStatusFlowController extends DesktopController
{
    ...
    public function index(Request $request): View|RedirectResponse
    {
        ...
        return view('knowledge.os-flow.index', [
            'pageTitle' => 'Status de OS',
            'statuses' => $statuses,
            'workflowGroups' => $this->buildStatusGroups($statuses),
        ]);
    }
    ...
    /**
     * Agrupa o catalogo de status por macrofase (`grupo_macro`), que e como a
     * tela de cadastro organiza as tabelas. Nao ha mais nada derivado de
     ...
     */
    private function buildStatusGroups(array $statuses): array
    {
        return collect($statuses)
            ...
            ->map(static function ($groupStatuses, string $grupoMacro): array {
                ...
                return [
                    ...
                    'status_count' => $groupStatuses->count(),
                    'active_status_count' => $groupStatuses
                        ->filter(static fn (array $status): bool => (bool) ($status['ativo'] ?? false))
                        ->count(),
                    'statuses' => $groupStatuses->all(),
                ];
            })
            ->values()
            ->all();
